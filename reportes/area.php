...
$link=Conectarse();
$sql2="SELECT fecha from validarfecha where usuario=$IDUSUARIO and tipo='area' ";
...
<form action="../actualizar/fecha-grafico-area.php" method="post">
...
<?php 
$sql="SELECT ar.nombre,count(*) as cantidad,ar.alias	from atencion as a  inner join ticket  as t on 	a.ate_tck_id=t.tck_id  inner join   webnets_rdgdb.usuario as  u  on
	t.tck_usuario=u.idusuario  inner join webnets_rdgdb.area as ar on
	u.area_idarea=ar.idarea inner join usuarios as user on 
	a.ate_usuario=user.id_usuario inner join  empresa as emp on
	t.tck_empresa=emp.codigoempresa inner join estado as est on
	...
	t.tck_tipo=ta.codigoactividad  where a.ate_estado like '03' and
ate_horaestado like '%$fecha%' 
group by  ar.nombre order by count(*) desc
...
<div class="row-clearfix">
<iframe src="/sistemas/graficos/barra-area.php" frameborder="0"
